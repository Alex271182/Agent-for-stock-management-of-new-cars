"""
GAC RF Stock Extractor  (v4)
================================
Запуск:  python gac_stock_extractor.py

Что делает:
1. Собирает ВСЕ автомобили GAC по всей РФ через API TradeDealer.
...
CONFIG = {
    # Базовые URL
    "site_url":   "https://gac.ru/cars/new/gac",
    "api_url":    "https://locator-backend.tradedealer.ru/filter",

    # ТОКЕНЫ платформы TradeDealer (постоянные для проекта GAC).
    ...
    # ВАЖНО: для GAC API filter?brands[]=gac БЕЗ models[] возвращает только
    # 7 машин (видимо, какой-то общий/служебный список). Чтобы получить ВСЕ
    # машины бренда (~1182 по всей РФ), необходимо итерировать по моделям
    # и для каждой делать filter?brands[]=gac&models[]=<alias>.
    #
    # Список alias-ов получен через запрос
    # https://locator-backend.tradedealer.ru/filter?brands[]=gac&models[]=<alias>
    # с подсчётом total. Полная сумма по этим 7 моделям = 1182.
    #
    # Если у GAC появится новая модель — добавь её alias сюда, иначе её
    # машины не попадут в сток. Узнать alias легко: открыть карточку модели
    # на gac.ru/models, в URL будет вида /models/<alias>.
    "models": [
        "gs4",
        "gs8",
        "gs8-fl",   # "Обновленный GS8"
        "m8-1",     # M8
        "s7",
        "aion-v",
        "empow",
    ...
    "timeout":       30,
    "max_pages":     500,    # ~1200/24 ≈ 143 страницы, 500 — с большим запасом
...
def _fetch_page_once(session, page, tokens, model=None):
    """Один запрос страницы, без retry.

    Если передан `model` (alias) — добавляет models[]=<model> к запросу.
    Это критично для GAC: без models[] API возвращает только 7 машин.
    """
    params = {
        "brands[]":      CONFIG["brand"],
        "order":         CONFIG["order"],
...
def fetch_page(session, page, tokens, model=None):
    ...
    for attempt in range(1, CONFIG["max_retries"] + 1):
        try:
            return _fetch_page_once(session, page, tokens, model=model), None
...
def fetch_all_cars(session, tokens):
    """Постранично собирает все автомобили GAC.

    Для GAC API возвращает машины только при указании models[]. Поэтому
    идём в два уровня:
      внешний цикл — по списку моделей из CONFIG["models"]
      внутренний цикл — пагинация результатов для каждой модели

    Все собранные машины объединяются в единый список. Дедупликация по
    car_id (если она нужна) делается на этапе UPSERT в Supabase.
    """
    all_cars = []
    debug_saved = False

    models = CONFIG["models"]
    print("[2/3] Загружаю каталог по моделям (limit={}/стр, моделей: {}) ...".format(
        CONFIG["page_limit"], len(models)))
    ...
    for model in models:
        ...
        while page <= CONFIG["max_pages"]:
            try:
                data, retry_exc = fetch_page(session, page, tokens, model=model)
            except requests.HTTPError as e:
                print("      ✗ HTTP-ошибка на стр {} модели {}: {}".format(page, model, e))
                if not debug_saved and e.response is not None:
                    try:
                        dbg = Path(CONFIG["output_dir"]) / "gac_debug_response.txt"
                        dbg.write_text(
                            "URL: {}\nStatus: {}\n\n{}".format(
                        ...
                        pass
                # При HTTP-ошибке прекращаем эту модель и идём к следующей
                break
            except Exception as e:
                print("      ✗ Ошибка стр {} модели {}: {}: {}".format(
                    page, model, type(e).__name__, e))
                break

            if data is None:
                print("      ✗ Стр {} модели {} не получена после {} попыток: {}".format(
                    page, model, CONFIG["max_retries"], retry_exc))
                break
            ...
            if not items:
                if page == 1:
                    # Это легально: модель может быть в CONFIG, но без машин
                    print("      ⚠ модель {}: 0 машин (total={})".format(model, total))
                break

            all_cars.extend(items)
            model_cars_count += len(items)
            print("      стр {:3d}: получено {:3d} а/м, всего по модели {:4d}/{}".format(
                page, len(items), model_cars_count, total))
        ...
        print("   ✓ модель {}: собрано {} машин".format(model, model_cars_count))
        time.sleep(CONFIG["delay_sec"])  # пауза между моделями

    print("[2/3] ✓ Всего собрано: {} машин по {} моделям".format(len(all_cars), len(models)))
    return all_cars
...
def save_csv(cars):
    ...
    out_path = Path(CONFIG["output_dir"]) / "gac_rf_stock_{}.csv".format(today)
